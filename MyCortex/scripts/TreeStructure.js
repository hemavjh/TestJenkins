...
//  }
//];
app = angular.module('treestructure', []).controller('MenuListControllers', [
  '$scope',
  '$filter',
  function ($scope, $filter) {
     ...
     // $scope.list = list;
      ...
      $scope.initCheckbox = function (item, parentItem) {
          return item.selected = parentItem && parentItem.selected || item.selected || true;
      };
...
app.filter('selected', [
  ...
  function ($filter) {
      return function (files) {
          return $filter('filter')(files, { selected: false });
      };
  }
]);
