...
.controller('AccordionController', ['$scope', '$attrs', 'accordionConfig', function ($scope, $attrs, accordionConfig) {
  ...
  tMyCortex.closeOthers = function(openGroup) {
    var closeOthers = angular.isDefined($attrs.closeOthers) ? $scope.$eval($attrs.closeOthers) : accordionConfig.closeOthers;
    if ( closeOthers ) {
      angular.forEach(tMyCortex.groups, function (group) {
        if ( group !== openGroup ) {
          group.isOpen = false;
        }
      });
  ...
  tMyCortex.addGroup = function(groupScope) {
    var that = this;
    tMyCortex.groups.push(groupScope);
  ...
  tMyCortex.removeGroup = function(group) {
    var index = tMyCortex.groups.indexOf(group);
    if ( index !== -1 ) {
      tMyCortex.groups.splice(tMyCortex.groups.indexOf(group), 1);
    }
  };
...
.directive('accordionGroup', ['$parse', '$transition', '$timeout', function($parse, $transition, $timeout) {
  return {
    ...
    controller: ['$scope', function($scope) {
      tMyCortex.setHeading = function(element) {
        tMyCortex.heading = element;
      };
    }],
...
dialogModule.provider("$dialog", function(){
  ...
  //      });
	tMyCortex.options = function(value){
		globalOptions = value;
	};

  // Returns the actual `$dialog` service that is injected in controllers
	tMyCortex.$get = ["$http", "$document", "$compile", "$rootScope", "$controller", "$templateCache", "$q", "$transition", "$injector",
  function ($http, $document, $compile, $rootScope, $controller, $templateCache, $q, $transition, $injector) {
    ...
    //     var d = new Dialog('path/to/dialog.html', MyDialogController);
      ...
      var self = this, options = tMyCortex.options = angular.extend({}, defaults, globalOptions, opts);

      tMyCortex.backdropEl = createElement(options.backdropClass);
      if(options.backdropFade){
        tMyCortex.backdropEl.addClass(options.transitionClass);
        tMyCortex.backdropEl.removeClass(options.triggerClass);
      }

      tMyCortex.modalEl = createElement(options.dialogClass);
      if(options.dialogFade){
        tMyCortex.modalEl.addClass(options.transitionClass);
        tMyCortex.modalEl.removeClass(options.triggerClass);
      }

      tMyCortex.handledEscapeKey = function(e) {
        if (e.which === 27) {
          self.close();
          e.preventDefault();
          self.$scope.$apply();
      ...
      tMyCortex.handleBackDropClick = function(e) {
        self.close();
        e.preventDefault();
        self.$scope.$apply();
      };
    ...
    Dialog.prototype.isOpen = function(){
      return tMyCortex._open;
    };

    // The `open(templateUrl, controller)` method opens the dialog.
    // Use the `templateUrl` and `controller` arguments if specifying them at dialog creation time is not desired.
    Dialog.prototype.open = function(templateUrl, controller){
      var self = this, options = tMyCortex.options;
      ...
      tMyCortex._loadResolves().then(function(locals) {
      ...
      tMyCortex.deferred = $q.defer();
      return tMyCortex.deferred.promise;
    };

    // closes the dialog and resolves the promise returned by the `open` method with the specified result.
    Dialog.prototype.close = function(result){
      var self = this;
      var fadingElements = tMyCortex._getFadingElements();
      ...
      tMyCortex._onCloseComplete(result);
    ...
    Dialog.prototype._getFadingElements = function(){
      var elements = [];
      if(tMyCortex.options.dialogFade){
        elements.push(tMyCortex.modalEl);
      }
      if(tMyCortex.options.backdropFade){
        elements.push(tMyCortex.backdropEl);
      }
    ...
    Dialog.prototype._bindEvents = function() {
      if(tMyCortex.options.keyboard){ body.bind('keydown', tMyCortex.handledEscapeKey); }
      if(tMyCortex.options.backdrop && tMyCortex.options.backdropClick){ tMyCortex.backdropEl.bind('click', tMyCortex.handleBackDropClick); }
    };

    Dialog.prototype._unbindEvents = function() {
      if(tMyCortex.options.keyboard){ body.unbind('keydown', tMyCortex.handledEscapeKey); }
      if(tMyCortex.options.backdrop && tMyCortex.options.backdropClick){ tMyCortex.backdropEl.unbind('click', tMyCortex.handleBackDropClick); }
    };

    Dialog.prototype._onCloseComplete = function(result) {
      tMyCortex._removeElementsFromDom();
      tMyCortex._unbindEvents();

      tMyCortex.deferred.resolve(result);
    };

    Dialog.prototype._addElementsToDom = function(){
      body.append(tMyCortex.modalEl);

      if(tMyCortex.options.backdrop) { 
        if (activeBackdrops.value === 0) {
          body.append(tMyCortex.backdropEl); 
        }
        activeBackdrops.value++;
      }

      tMyCortex._open = true;
    };

    Dialog.prototype._removeElementsFromDom = function(){
      tMyCortex.modalEl.remove();

      if(tMyCortex.options.backdrop) { 
        activeBackdrops.value--;
        if (activeBackdrops.value === 0) {
          tMyCortex.backdropEl.remove(); 
        }
      }
      tMyCortex._open = false;
    };
    ...
    Dialog.prototype._loadResolves = function(){
      ...
      if (tMyCortex.options.template) {
        templatePromise = $q.when(tMyCortex.options.template);
      } else if (tMyCortex.options.templateUrl) {
        templatePromise = $http.get(tMyCortex.options.templateUrl, {cache:$templateCache})
        .then(function(response) { return response.data; });
      }

      angular.forEach(tMyCortex.options.resolve || [], function(value, key) {
        keys.push(key);
...
.controller('TabsController', ['$scope', '$element', function($scope, $element) {
  ...
  tMyCortex.select = $scope.select = function selectPane(pane) {
  ...
  tMyCortex.addPane = function addPane(pane) {
    if (!panes.length) {
      $scope.select(pane);
    }
    panes.push(pane);
  ...
  tMyCortex.removePane = function removePane(pane) { 
