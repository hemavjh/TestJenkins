...
namespace MyCortex.Repositories
{
    interface ICarecoordinatorRepository
    {
        ...
        IList<CareCoordinatorModel> CareCoordinator_FilterPatientList(long Coordinator_Id, string PATIENTNO, string INSURANCEID, string NATIONALITY_ID, string MOBILE_NO, string EMAILID, long UserTypeId, string FIRSTNAME, string LASTNAME, string MRN, int TypeId, Guid Login_Session_Id);
        IList<CareGiverListModel> CareGiver_List(long Id);
        List<AssignCareGiverModel> Assign_CareGiver(AssignCareGiverModel obj);
        IList<GetParameterValueModel> Get_ParameterValue(long PatientId, long UserTypeId, Guid Login_Session_Id, int StartRowNumber, int EndRowNumber, int AlertType);
