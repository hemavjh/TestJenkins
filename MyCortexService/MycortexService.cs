                    ...
                    if (dt.Rows.Count > 0)
                    {
                        try
                        {
                            for(int i=0; i<dt.Rows.Count; i++) { 
                                Id = Convert.ToInt64(dt.Rows[i]["MOBILE_NO"].ToString());
                                ...
                                re.emiratesId = Emirates_Id;
                                re.consultationCategoryId = consultationCategoryId;
                                re.clinicianLicense = Clinician_Licence;
                                re.mobileNumber = MobileNumber;
                                re.payerId = payerId;
                                re.serviceCategoryId = serviceCategoryId;
                                re.countryCode = CountryCode;
                                re.facilityLicense = "MF2007";
                                JObject strJPostData =(JObject) JsonConvert.SerializeObject(re, Newtonsoft.Json.Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                                EligibilityCheckController checkELigibility = new EligibilityCheckController();
                                checkELigibility.AddEligibilityEequest(strJPostData);
                            }
                        }
                        catch (Exception ex)
                        {
